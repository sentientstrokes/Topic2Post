...
from src.scraper.parser import clean_html, validate_text # Import the functions
...
async def fetch_valid_articles(urls: List[str], max_count: int = 3) -> List[Article]:
    ...
    for i, url in enumerate(urls):
        ...
        try:
            ...
            article_data = await crawla4ai_client.run(url=url)
            html_content = article_data.get('html')
            fetched_url = article_data.get('url', url)
